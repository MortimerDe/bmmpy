...
public:
    ...
    std::size_t words_per_row() const noexcept { return (_cols + k_word_bits - 1) / k_word_bits; }
    std::size_t total_words() const noexcept { return _total_words; }
    std::size_t total_bytes() const noexcept { return _total_words * sizeof(std::uint64_t); }
    ...
    void save_text(const std::filesystem::path& path) const;
    void save_binary(const std::filesystem::path& path) const;

    static BitMatrix load_text(std::istream& in);
    static BitMatrix load_binary(std::istream& in);

    static BitMatrix load_text(const std::filesystem::path& path);
    static BitMatrix load_binary(const std::filesystem::path& path);
...
private:
    ...
    std::uint64_t* row_ptr_unchecked(std::size_t row) noexcept { return _data + row * _stride; }
